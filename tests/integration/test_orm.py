...
def test_order_lines_mapper_can_save_lines(session):
...
def test_retrieving_batches(session):
    ...
    session.execute(
        'INSERT INTO batches (reference, sku, _purchased_quantity, eta) VALUES'
        '("batch2", "sku2", 200, "2020-10-14")'
    )
    expected = [
        model.Batch('batch1', 'sku1', 100, eta=None),
        model.Batch('batch2', 'sku2', 200, eta=date(2020, 10, 14))
    ]
...
def test_retrieving_allocations(session):
    session.execute('INSERT INTO order_lines (orderid, sku, qty) VALUES ("order1", "sku1", 12)')
